...
class Doc_most_sim():
    ...
    def export_data_list(self,commandText):
        ...
        for item in self.db.find(commandText):
            i += 1
            std_model_info=item['std_model_info']
            list1.append(std_model_info)
        return list1

    def export_data(self,commandText):
        i = 0
        info={}
        for item in self.db.find(commandText):
            i += 1
            model_id=item['model_id']
            model_standard_name=item['model_standard_name']
            model_factory=item['model_factory']
            model_brand=item['model_brand']
            model_cate_name=item['model_cate_name']
            # std_model_info=item['std_model_info']
    ...
    def extract_most_sim_content(self, sentences):
        ...
        for index, sentence in enumerate(sentences):
            ...
            if flag:
                value='{}'.format(senti_words[0])
                commandText={'ggid':value}
                data_list=self.export_data_list(commandText)
                ...
                if flag:
                    ...
                    for k in range(len(senti_words)):
                        ...
                        commandText = {'model_cate_name': value}
                        data_list = self.export_data_list(commandText)
                        for j in range(len(data_list)):
                            content = str(data_list[j])
                            id2content[len(id2content)] = content
        ...
        try:
            ...
            if float(max_sim)>0.8:
                ...
                value = '{}'.format(result)
                commandText = {'std_model_info': value}
                info=self.export_data(commandText)
                if info:
                    return info
                else:
                    return '数据库里没有这种车型！抱歉'
            else:
                return '您提供的信息不完整！抱歉'
        except :
            return '抱歉'
...
# Create server
server=ThreadXMLRPCServer(("10.9.1.199", 8890),requestHandler=RequestHandler)
